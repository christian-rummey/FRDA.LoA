# descr -------------------------------------------------------------------
project. <- 'LoA'
out.     <- 'output.plots.tte.i'
desc.    <- { 
  'output plots for 3 Tables in Paper'
}
# last change
# 2019-10-05
# require -----------------------------------------------------------------
...
result.i <- read_rds("DATA derived/result.i.age.all.rds")
fits.i   <- read_rds("DATA derived/fits.i.age.all.rds")
...
vs <- c(res.tmp %>% filter(paramcd %in% c('fane7')))$median
